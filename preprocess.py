...
for folder in folders:
    ...
    pretrained_model = models.resnet50(weights = 'ResNet50_Weights.DEFAULT')  # Use updated weights argument
    ...
    def preprocess_and_extract_vectors(image_path):
        ...
        preprocess = transforms.Compose([
            transforms.Resize((224, 224)),  # Resize to match the pretrained model's input size
    ...
    n = int(len(os.listdir(data_dir)) / 36)

    total_data_points = n
    images_per_data_point = 36

    # # Initialize an empty NumPy array to store the data
    data = np.empty((total_data_points, images_per_data_point, 2048), dtype=np.float32)  # Assuming ResNet-50 outputs 2048-dimensional vectors
    ...
    np.save(f'preprocessed_{folder}.npy', data)
